...
from algorithms.pencilSketch import *
from algorithms.pencilSketch_GPU import *

class pencilSketch_GUI:
    def __init__(self, root, imagePath, work_width, work_height, effectName):
        ...
        pencil_sketch(self.mainCanvas.ImagePath, cv2.cvtColor(np.array(self.mainCanvas.PIL_image), cv2.COLOR_RGB2BGR),
                      3, 1)
    ...
    def draw_GUI(self):
        ...
        # Sigma label
        self.sigmaLabel = tk.Label(self.mainCanvas.adjustment_area, text="Sigma: 1", font=Font(self.mainCanvas.adjustment_area, size=12))
        self.sigmaLabel.place(x=adjust_width // 4, y=adjust_height // 8 + self.slider.winfo_height() + 5)
        self.mainCanvas.root.update()
        # Sigma label
        self.sigmaSlider = tk.Scale(self.mainCanvas.adjustment_area, length=adjust_width // 2, width=adjust_height // 8, orient=tk.HORIZONTAL, from_=1, to=100, showvalue=0, command=self.sigma_slide)
    ...
    def sigma_slide(self, n):
        self.sigmaLabel['text'] = f"Sigma: {n}"
        self.real_time_process()
    ...
    def processImage(self):
        if (not self.IS_GPU_OPTIMIZED):
            pencilSketch_ = pencilSketch(cv2.cvtColor(np.array(self.mainCanvas.PIL_image), cv2.COLOR_RGB2BGR))
            results = pencilSketch_.pencil_sketch((int)(self.slider.get()), (int)(self.sigmaSlider.get()))
    ...
    def real_time_process(self):
        if (self.IS_GPU_OPTIMIZED):
            results = pencil_sketch(self.mainCanvas.ImagePath, cv2.cvtColor(np.array(self.mainCanvas.PIL_image), cv2.COLOR_RGB2BGR), (int)(self.slider.get()), (int)(self.sigmaSlider.get()))
            PIL_Image = Image.fromarray(results)
            photo_result = ImageTk.PhotoImage(PIL_Image)
            self.mainCanvas.render_result_image(photo_result)
